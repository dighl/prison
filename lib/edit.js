"use strict";
define(['lib/vendor/FileSaver'], function(saveAs){
  function editDistSimple(){
    ...
    if(stringA.indexOf(' ') != -1 || stringB.indexOf(' ') != -1){
      ...
      alignments = editList(seqA,seqB);
    }else{
      alignments = editDist(stringA,stringB);
    }
  ...
  function editAll()
  {
    ...
    for(i=0;i<stringsAtmp.length;i++){
      tmp = stringsAtmp[i].replace(/\s+/,' ');
      if(tmp !== ''){
        stringsA.push(tmp);
      }
    }
    for(i=0;i<stringsBtmp.length;i++){
      tmp = stringsBtmp[i].replace(/\s+/,' ');
      if(tmp !== ''){
        stringsB.push(tmp);
      }
    }
    ...
    for(var i=0;i<stringsA.length;i++){
      ...
      if(stringA.indexOf(' ') != -1 || stringB.indexOf(' ') != -1){
        ...
        alms = editList(seqA,seqB);
      }else{
        alms = editDist(stringA,stringB);
      }
  ...
  function multiple(seqs){
    //Indices to use inside this function:
    var i = 0;
    ...
    var gaps = [];
    for(i=0; i<=reference.length; i++){ gaps.push(0); }
    ...
    seqs.forEach(function(seq, i){
      if(i != idx){
        ...
        var counter = 0;
        var gcount = 0;
        alms[0].forEach(function(segment){
          if(segment == '-'){
            gcount += 1;
