...
define(['lib/highlight'], function(highlight){
  ...
  function multiple(seqs){
    ...
    seqs.forEach(function(seq, i){
      if(i != idx){
        ...
        var counter = 0;
        var gcount = 0;
        alms[0].forEach(function(segment, j){
          if(segment == '-'){
            gcount += 1;
  ...
  function tokens2class(tokens){
    ...
    for(var i=0; i<tokens.length; i++){
      out.push(highlight.getSoundClass(tokens[i]));
    }
    return out;
  }
  ...
  return {
    'editList': editList,
    'multiple': multiple,
    'tokens2class': tokens2class,
    'class2tokens': class2tokens,
    'scalign': scalign
  };
});
